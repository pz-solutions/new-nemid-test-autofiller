<template>
  <el-form
    ...
    class="demo-dynamic"
  >
    <el-table ref="singleTable" :data="cprs" style="width: 100%">
      <el-table-column property="selected" width="30">
        <template slot-scope="scope">
          <el-checkbox
            v-if="scope.row"
            @input="select(scope.row.id)"
            :value="isSelected(scope.row.id)"
          ></el-checkbox>
        </template>
      </el-table-column>
      ...
      <el-table-column property="password" label="Password" width="140">
        <template slot-scope="scope">
          <el-input
            v-if="scope.row"
            v-model.lazy="scope.row.password"
